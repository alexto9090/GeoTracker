...
class InfoVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        Geo.getGeoIp { (info, error) in
            if error == nil {
                if let geo = info { DispatchQueue.main.async { self.setData(geo) } }
            }
        }
    ...
    func setData(_ info: Geo.Info) {
        ...
        (view.viewWithTag(105) as? UILabel)?.text = "\(info.lat ?? 0.0)"
        (view.viewWithTag(106) as? UILabel)?.text = "\(info.lon ?? 0.0)"
